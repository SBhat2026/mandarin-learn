...
export const api = STATIC ? {
  ...
  readingProfile: async () => ({ charactersMet: 0, estimatedCoverage: 0, band: 'too-hard', highYield: [] }),
  charInsight: async (term) => ({ insight: null, results: (await demoLookup(term)).results }),
  tone: () => demo('tone.json'),
  ...
  readingProfile: () => req('/api/reading/profile'),
  charInsight: (hanzi) => req('/api/reading/char?hanzi=' + encodeURIComponent(hanzi)),
