...
const firstReading = (pinyinJson) => { try { return JSON.parse(pinyinJson || '[]')[0] || ''; } catch { return ''; } };

// Meaning-bearing radicals only. Structural/stroke radicals (一, 丨, 亅…) carry no
// semantic cue, so teaching "一 means one → 不" would mislead. Curated meanings
// keep the semantic family lesson accurate.
const SEMANTIC_RADICALS = {
  ...
  '刂': 'knife', '刀': 'knife', '雨': 'rain', '宀': 'roof/home', '广': 'shelter', '页': 'head', '耳': 'ear',
};

const INITIALS = ['zh', 'ch', 'sh', 'b', 'p', 'm', 'f', 'd', 't', 'n', 'l', 'g', 'k', 'h', 'j', 'q', 'x', 'z', 'c', 's', 'r', 'y', 'w'];
// Rhyme = toneless syllable minus its leading initial — the part that a phonetic
// series actually predicts. Used to verify a phonetic component really applies.
function rhyme(pinyin = '') {
